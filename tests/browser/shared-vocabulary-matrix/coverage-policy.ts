import type { SharedVocabularyClassSupport } from "../../../packages/core/src/types.js";
import {
  matrixCoverageFamilyForClass,
  sharedVocabularyMatrixCoverageFamilies,
} from "../../../packages/core/src/shared-vocabulary-generated-families.js";
import { isEnforcedSharedVocabularyLevel } from "../../../packages/core/src/shared-vocabulary-levels.js";
import type { SharedVocabularySupportLevel } from "./types.js";

export interface SharedVocabularyCoverageViolation {
  message: string;
}

export function coverageExceptionForClass(className: string, level: SharedVocabularySupportLevel) {
  return matrixCoverageFamilyForClass(className, level);
}
  ...
  for (const className of gatedSupportClasses) {
    ...
    if (!directlyCovered) {
      for (const entry of supportEntries) {
        const exception = coverageExceptionForClass(className, entry.level);
        if (!exception) {
          violations.push({
...
export { sharedVocabularyMatrixCoverageFamilies };
