...
test.describe("player match interactions", () => {
  test("creates match pairs", async ({ page }) => {
    ...
    await expect(page.locator("qti-assessment-item-player .qti3-match-source-bank")).toContainText(
      "Mulched beds stayed moist two days longer after watering.",
    );
    await expect(page.locator("qti-assessment-item-player .qti3-match-source")).toHaveCount(4);
    await expect(page.locator("qti-assessment-item-player .qti3-match-target")).toHaveCount(4);
    ...
    await expect(
      ...
      "Mulched beds stayed moist two days longer after watering. to Mulch helped reduce water loss from the soil.",
    );
  });
});
