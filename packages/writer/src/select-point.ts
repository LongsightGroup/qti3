...
export function renderQti3SelectPointItem(input: Qti3SelectPointBuilderInput): string {
  ...
  const cardinality = selectPointCardinality(input);
...
`
    : "";
  const areaMappingXml = areaMappingBlock(input.targets);
  const declarationsXml = `  <qti-response-declaration identifier="${escapedResponseIdentifier}" cardinality="${cardinality}" base-type="point">
${correctResponseXml}${areaMappingXml}  </qti-response-declaration>`;
...
function selectPointCardinality(input: Qti3SelectPointBuilderInput): "single" | "multiple" {
