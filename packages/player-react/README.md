...
`@longsightgroup/qti3-player`. This package only handles React lifecycle wiring, typed callbacks,
...
- Keep `messageCatalog`, `fetchXml`, `resolveAsset`, `resolveStylesheet`, and `loadOptions` object
  literals stable across renders (`useMemo` / `useCallback`). New references on every render cause
  unnecessary resyncs or reloads.
- URL loading stays imperative: `ref.current?.loadUrl(url, loadOptions)`.

Local manual proof: from the repo root run `pnpm dev:adapter-react` and open
`/adapter-react.html`.
