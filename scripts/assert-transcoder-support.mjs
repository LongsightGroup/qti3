...
const reportIndexPath = join(root, "packages", "transcoder", "SUPPORT.md");
const supportDir = join(root, "packages", "transcoder", "support");
...
if (process.argv.includes("--write")) {
  ...
  await writeFile(reportIndexPath, supportIndex(profiles), "utf8");
  for (const profile of profiles) {
    const entries = observations.filter((entry) => entry.caseId.startsWith(`${profile}/`));
    await writeFile(profileSupportPath(profile), profileSupportReport(profile, entries), "utf8");
  }
} else {
  let locked = [];
  try {
  ...
  const committedIndex = await readFile(reportIndexPath, "utf8").catch(() => "");
  if (committedIndex !== supportIndex(profiles)) {
    failures.push("packages/transcoder/SUPPORT.md is stale.");
  }
  for (const profile of profiles) {
    const entries = observations.filter((entry) => entry.caseId.startsWith(`${profile}/`));
    const path = profileSupportPath(profile);
    const committedReport = await readFile(path, "utf8").catch(() => "");
    if (committedReport !== profileSupportReport(profile, entries)) {
      failures.push(`${relativeSupportPath(profile)} is stale.`);
      continue;
    }
    for (const entry of entries) {
      if (!committedReport.includes(`\`${entry.sha256}\``)) {
        failures.push(`${relativeSupportPath(profile)} is missing hash for ${entry.caseId}.`);
      }
    }
  }
}
...
function supportIndex(profileIds) {
  const links = profileIds
    .map((profile) => `- [\`${profile}\`](./support/${profile}.md)`)
    .join("\n");
...
function profileSupportReport(profileId, entries) {
...
`,
  ...
  return `# Transcoder support report: \`${profileId}\`

${cases.join("\n").trimEnd()}
`;
}

function profileSupportPath(profileId) {
  return join(supportDir, `${profileId}.md`);
}

function relativeSupportPath(profileId) {
  return `packages/transcoder/support/${profileId}.md`;
}
